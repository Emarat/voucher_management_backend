  ...
  try {
    const getDataQuery = `
      ...
      FROM
        requisition_master_data AS master
      LEFT JOIN
        requisition_details_data AS details ON master.requisition_master_id = details.requisition_master_id
      LEFT JOIN
        requisition_status AS rs ON master.requisition_master_id = rs.requisition_master_id
      LEFT JOIN
        comments ON master.requisition_master_id = comments.requisition_master_id
      LEFT JOIN
        requisition_file_details AS files ON master.requisition_master_id = files.requisition_master_id
      LEFT JOIN
        customers ON master.customer_id = customers.customer_id
      LEFT JOIN
        vendor ON master.supplier_id = vendor.vendor_id
      LEFT JOIN
        status ON rs.status_id = status.status_id
      LEFT JOIN
        category ON details.item_category_id = category.category_id
      LEFT JOIN
        products ON details.item_name = products.id;
    `;
