...
// Function to get tasks for a process instance
async function getTasks(processInstanceId) {
  ...
  // console.log(
  //   `Found ${tasks.length} task(s) assigned to user ${req.body.userId}`
  // );
  console.log(tasks);
...
router.post('/start/:key', async (req, res) => {
  try {
    ...
    const tasks = await getTasks(processInstanceId);
